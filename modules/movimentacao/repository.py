...
class MovimentacaoRepository():
    db = DataBase()
    QUERY_MOVIMENTACOES = """SELECT id, bem_id, setor_origem_id, setor_destino_id, data, ativo
                          FROM movimentacoes WHERE ativo = TRUE"""
    QUERY_MOVIMENTACOES_ID = """SELECT id, bem_id, setor_origem_id, setor_destino_id, data, ativo FROM movimentacoes WHERE id = %s"""
    QUERY_CREATE_MOVIMENTACOES = """INSERT INTO movimentacoes (bem_id, setor_origem_id, setor_destino_id, data)
                                    VALUES (%s, %s, %s, %s, TRUE)RETURNING id, bem_id, setor_origem_id, setor_destino_id, data;"""
    QUERY_PUT_MOVIMENTACAO = """UPDATE movimentacoes SET data = %s, setor_origem_id = %s WHERE id = %s 
